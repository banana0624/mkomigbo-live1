    ...
    return new class {
      ...
      public function render(array $items): string {
        ...
        foreach ($items as $it) {
          ...
          if ($src !== '') $meta[] = $src;
          $yr = trim((string)($it['pub_year'] ?? ''));
          if ($yr !== '') $meta[] = $yr;

          $meta_txt = $meta ? (" <span class=\"mk-attachments__meta\">" . $h(implode(" • ", $meta)) . "</span>") : "";
          $ext = !empty($it['is_external']) ? " target=\"_blank\" rel=\"noopener noreferrer\"" : "";

          $html .= "    <li class=\"mk-attachments__item\">";
          $html .= "<a class=\"mk-attachments__link\" href=\"" . $h($href) . "\"{$ext}>" . $h($label) . "</a>";
          $html .= $meta_txt;
          $html .= "</li>\n";
        }
