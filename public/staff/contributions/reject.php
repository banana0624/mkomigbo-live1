...
// /public/staff/contributions/reject.php
...
if (!mk_contrib_staff_csrf_verify($token)) {
 ...
 $uid = (int)($_SESSION['staff_user_id'] ?? 0);
    ...
    try {
        mk_contribution_set_status($id, 'rejected', $uid, $note !== '' ? $note : null);

    if (function_exists('mk_staff_audit_log')) {
        mk_staff_audit_log('contribution_rejected', [
            'contribution_id' => $id,
        ], $uid);
    }

    mk_staff_flash_set('success', 'Contribution rejected successfully.');
    header('Location: /staff/contributions/show.php?id=' . $id, true, 302);
    exit;
} catch (Throwable $e) {
    mk_staff_flash_set('error', 'Rejection failed.');
    header('Location: /staff/contributions/show.php?id=' . $id, true, 302);
    exit;
}