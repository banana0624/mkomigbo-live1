...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
  ...
  if ($has_menu_name && $has_name) {
    ...
    $errors[] = 'The subjects table has no usable name columns.';
  }
  ...
  if (!$errors) {
    try {
      ...
      if (!$cols) {
        throw new RuntimeException('No insertable columns detected on subjects.');
      }
      ...
      pf__flash_set('success', "Created subject #{$new_id} successfully.");
...
$page_title = 'New Subject • Staff';
$page_desc  = 'Create a new subject and set its core metadata.';
$nav_active = 'subjects';
$active_nav = 'subjects';
...
<section class="staff-pagehead">
  <h1 class="staff-pagehead__title">New subject</h1>
  <p class="staff-pagehead__desc">Create a new subject and control its naming, slug, order, and visibility.</p>
</section>
  ...
  <div class="staff-card">
    <div class="staff-card__body">
      <div class="staff-actions" style="justify-content:space-between;align-items:flex-start;">
        <div>
          <h2 style="margin:0;font-size:1.15rem;">Create subject</h2>
          <p style="margin:6px 0 0;color:#667085;">Use the fields your schema supports. Blank optional fields can be left empty.</p>
        </div>
      ...
      <form method="post" action="" style="margin-top:14px;display:grid;gap:14px;">
        ...
        <?php if ($has_slug): ?>
          <div>
            ...
            <input id="slug" type="text" name="slug" value="<?= h($slug) ?>" placeholder="auto-generated if blank" style="width:100%;min-height:44px;padding:10px 12px;border:1px solid rgba(17,24,39,.14);border-radius:12px;">
            <div style="margin-top:6px;color:#667085;font-size:.92rem;">Used in public URLs where applicable.</div>
          </div>
        <?php endif; ?>
        ...
        <?php if ($order_col): ?>
          <div>
            ...
            <input id="nav_order" type="text" name="nav_order" value="<?= h($order_raw) ?>" placeholder="blank allowed" style="width:100%;min-height:44px;padding:10px 12px;border:1px solid rgba(17,24,39,.14);border-radius:12px;">
          </div>
        <?php endif; ?>
        ...
        <?php if ($has_is_public): ?>
          <label style="display:flex;gap:10px;align-items:flex-start;">
            ...
            <span><strong>Public visibility</strong><br><span style="color:#667085;">Make this subject visible on the public side.</span></span>
          </label>
        <?php endif; ?>

        <div class="staff-actions">
          <button type="submit" class="staff-chip" style="cursor:pointer;">Create subject</button>
