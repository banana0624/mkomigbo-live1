...
$page_desc  = 'Manage contributor profiles, status, visibility, ordering, and profile details.';
...
if ($q !== '') {
  $where[] = '(display_name LIKE :q OR slug LIKE :q OR email LIKE :q)';
  $params[':q'] = '%' . $q . '%';
}
...
$sql = "
  SELECT id, slug, display_name, email, roles, status, is_public, sort_order, deleted_at, created_at, updated_at
  FROM contributors
...
$return = '/staff/contributors/index.php' . $return_qs;
...
<section class="staff-pagehead">
  <h1 class="staff-pagehead__title">Contributors</h1>
  <p class="staff-pagehead__desc">Manage contributor profiles, visibility, status, ordering, and editorial identity.</p>
</section>
    ...
    <div class="staff-card__body">
      <form method="get" class="staff-actions" style="align-items:flex-end;">
        <div style="flex:1 1 280px;">
          ...
          <input id="q" type="text" name="q" value="<?= h($q) ?>" placeholder="Search display name, slug, email" style="width:100%;min-height:44px;padding:10px 12px;border:1px solid rgba(17,24,39,.14);border-radius:12px;">
        </div>
        ...
        <div class="staff-actions">
          ...
          <a class="staff-chip" href="<?= h($u('/staff/contributors/new.php')) ?>">New contributor</a>
        </div>
      </form>
    </div>
  </div>
...
<section class="staff-section">
  <form method="post" action="<?= h($u('/staff/contributors/bulk.php')) ?>">
    <?= pf__csrf_field() ?>
    ...
    <div class="staff-card">
      <div class="staff-card__body">
        <div class="staff-actions" style="justify-content:space-between;align-items:flex-start;">
          <div>
            <h2 style="margin:0;font-size:1.15rem;">Contributor registry</h2>
          ...
          <table style="width:100%;border-collapse:collapse;min-width:980px;">
            <thead>
              <tr style="background:rgba(17,24,39,.04);">
                ...
                <th style="text-align:left;padding:12px 14px;border-bottom:1px solid rgba(17,24,39,.10);">Email</th>
            ...
            <?php if (!$rows): ?>
              <tr>
                <td colspan="8" style="padding:16px 14px;color:#667085;">No contributors found.</td>
              </tr>
            <?php else: ?>
              <?php foreach ($rows as $r): ?>
                <?php
                  $id   = (int)$r['id'];
                  $show = '/staff/contributors/show.php?id=' . rawurlencode((string)$id) . '&return=' . rawurlencode($return);
                  $edit = '/staff/contributors/edit.php?id=' . rawurlencode((string)$id) . '&return=' . rawurlencode($return);
                  $del  = '/staff/contributors/delete.php?id=' . rawurlencode((string)$id) . '&return=' . rawurlencode($return);
                ?>
                <tr>
                  ...
                  <td style="padding:12px 14px;border-bottom:1px solid rgba(17,24,39,.08);"><?= h((string)($r['email'] ?? '')) ?></td>
