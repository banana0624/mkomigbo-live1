...
try {
  ...
  $slug = mk_unique_contributor_slug($pdo, $slug_base);

  $bio_html = safe_bio_html($bio_raw);

  $st = $pdo->prepare("
    INSERT INTO contributors
      (slug, display_name, email, roles, status, is_public, sort_order, bio_raw, bio_html, created_at, updated_at, deleted_at)
    VALUES
      (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(), NULL)
  ");
  $st->execute([
  ...
  flash_set('notice', 'Contributor created successfully.');
