<?php
declare(strict_types=1);
echo '<div class="mk-prose">';
echo '<h2>What to Read, and Why</h2>';

echo '<h3>Primary Sources</h3>';
echo '<ul>';
echo '<li><strong>Olaudah Equiano, <em>The Interesting Narrative of the Life of Olaudah Equiano</em> (1789)</strong> — The foundational primary source on 18th-century Igbo life and the slave trade. Available free at Project Gutenberg.</li>';
echo '<li><strong>Nnamdi Azikiwe, <em>My Odyssey: An Autobiography</em> (1970)</strong> — The independence-era nationalist leader\'s own account of his formation and political career.</li>';
echo '<li><strong>Chinua Achebe, <em>There Was a Country: A Personal History of Biafra</em> (2012)</strong> — The definitive Igbo literary account of the war and its aftermath. Essential.</li>';
echo '</ul>';

echo '<h3>Archaeology and Origins</h3>';
echo '<ul>';
echo '<li><strong>Thurstan Shaw, <em>Igbo-Ukwu: An Account of Archaeological Discoveries in Eastern Nigeria</em> (2 vols., 1970)</strong> — The definitive account of the excavations. Technical but indispensable.</li>';
echo '<li><strong>Don Ohadike, <em>Anioma: A Social History of the Western Igbo People</em> (1994)</strong> — Important regional history with strong archaeological and oral tradition material.</li>';
echo '</ul>';

echo '<h3>General Igbo History</h3>';
echo '<ul>';
echo '<li><strong>Elizabeth Isichei, <em>A History of the Igbo People</em> (1976)</strong> — The most comprehensive academic history of the Igbo, covering origins through the post-civil war period. The starting point for serious study.</li>';
echo '<li><strong>Adiele Afigbo, <em>Ropes of Sand: Studies in Igbo History and Culture</em> (1981)</strong> — Essential essays on Igbo political history, the Nri Kingdom, and the colonial encounter.</li>';
echo '<li><strong>Victor Uchendu, <em>The Igbo of Southeast Nigeria</em> (1965)</strong> — Anthropological account of Igbo social organisation, religion, and culture. Foundational.</li>';
echo '</ul>';

echo '<h3>The Slave Trade</h3>';
echo '<ul>';
echo '<li><strong>G. Ugo Nwokeji, <em>The Slave Trade and Culture in the Bight of Biafra</em> (2010)</strong> — The definitive modern study of the Igbo experience in the Atlantic slave trade.</li>';
echo '<li><strong>Adiele Afigbo, <em>The Warrant Chiefs: Indirect Rule in Southeastern Nigeria 1891–1929</em> (1972)</strong> — Essential on the colonial administrative system and its relationship to the Aro trading network.</li>';
echo '</ul>';

echo '<h3>Colonial Resistance</h3>';
echo '<ul>';
echo '<li><strong>Judith Van Allen, "Sitting on a Man: Colonialism and the Lost Political Institutions of Igbo Women" (1972)</strong> — The foundational academic account of the Women\'s War of 1929. Available in African Studies Review.</li>';
echo '<li><strong>Don Ohadike, <em>The Ekumeku Movement: Western Igbo Resistance to the British Conquest of Nigeria, 1883–1914</em> (1991)</strong> — The only book-length study of the Ekumeku resistance movement.</li>';
echo '</ul>';

echo '<h3>The Biafra War</h3>';
echo '<ul>';
echo '<li><strong>Frederick Forsyth, <em>The Biafra Story</em> (1969)</strong> — Contemporary journalistic account from the Biafran side. Invaluable as contemporaneous witness.</li>';
echo '<li><strong>John Stremlau, <em>The International Politics of the Nigerian Civil War 1967–1970</em> (1977)</strong> — The standard academic account of the international dimensions of the war.</li>';
echo '<li><strong>Chimamanda Ngozi Adichie, <em>Half of a Yellow Sun</em> (2006)</strong> — The most widely read fictional account of the war and its human cost.</li>';
echo '<li><strong>Axel Harneit-Sievers et al. (eds.), <em>A Social History of the Nigerian Civil War</em> (1997)</strong> — Essays on the war\'s social dimensions, including the famine and civilian experience.</li>';
echo '</ul>';

echo '<h3>Contemporary Igbo</h3>';
echo '<ul>';
echo '<li><strong>Axel Harneit-Sievers, <em>Constructions of Belonging: Igbo Communities and the Nigerian State since Independence</em> (2006)</strong> — The best recent academic account of post-war Igbo politics and community organisation.</li>';
echo '<li><strong>Wale Adebanwi & Ebenezer Obadare (eds.), <em>Encountering the Nigerian State</em> (2010)</strong> — Essays on contemporary Nigerian politics including Igbo political dynamics.</li>';
echo '</ul>';


echo '<h2 style="margin-top:28px;">Key Texts — Free Downloads</h2>';
echo '<ul>';
echo '<li><a href="https://archive.org/details/thingsfallapart00ache" target="_blank"><strong>Things Fall Apart — Chinua Achebe</strong></a> (1958) — archive.org</li>';
echo '<li><a href="https://archive.org/details/arrowofgod00ache" target="_blank"><strong>Arrow of God — Chinua Achebe</strong></a> (1964) — archive.org</li>';
echo '<li><a href="https://archive.org/search?query=elizabeth+isichei+igbo+people" target="_blank"><strong>Elizabeth Isichei — History of the Igbo People</strong></a> — archive.org</li>';
echo '<li><a href="https://archive.org/details/historynigeria00burn" target="_blank"><strong>History of Nigeria — Alan Burns</strong></a> (1929) — archive.org [FREE]</li>';
echo '<li><a href="https://archive.org/search?query=UNESCO+general+history+africa" target="_blank"><strong>UNESCO General History of Africa (8 vols)</strong></a> — archive.org [FREE]</li>';
echo '<li><a href="https://www.africabib.org" target="_blank"><strong>Africa Bibliography</strong></a> — comprehensive bibliography of African studies</li>';
echo '</ul>';


echo '<h2 style="margin-top:28px;">Free Digital Libraries</h2>';
echo '<ul>';
echo '<li><a href="https://archive.org" target="_blank"><strong>Internet Archive</strong></a> — millions of free books; search any author or title; download PDF, ePub, Kindle</li>';
echo '<li><a href="https://www.globalgreyebooks.com" target="_blank"><strong>Global Grey Ebooks</strong></a> — free PDF and ePub of classic texts, beautifully formatted</li>';
echo '<li><a href="https://www.gutenberg.org" target="_blank"><strong>Project Gutenberg</strong></a> — free public domain books</li>';
echo '<li><a href="https://www.sacred-texts.com" target="_blank"><strong>Internet Sacred Text Archive</strong></a> — sacred and esoteric texts from all traditions</li>';
echo '<li><a href="https://www.holybooks.com" target="_blank"><strong>HolyBooks.com</strong></a> — free PDF downloads of religious and spiritual texts</li>';
echo '<li><a href="https://www.jstor.org" target="_blank"><strong>JSTOR</strong></a> — academic articles; 100 free per month with free registration</li>';
echo '</ul>';

echo '</div>';