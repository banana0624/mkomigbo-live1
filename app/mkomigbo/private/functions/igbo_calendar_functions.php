...
/**
 * /private/igbo-calendar/igbo_calendar_functions.php
 *
 * Igbo Calendar engine + server renderer helpers (4 columns).
 *
 * Properties:
 * - Leap year = Gregorian leap year
 * - Year start = 3rd week Feb new moon window (approx)
 * - 13 months, 28-day months, Month 7 = 29, Leap year Month 1 = 29
 * - Marketday anchor checkpoint: 2026-01-07 = Nkwo
 * - Moon: calibrated illumination + stage + 28-frame sprite index
 * - TODAY marked with .mk-cal-cell--today + badge
 *
 * Notes:
 * - This file only defines functions; it does NOT emit a full HTML page.
 * - Your public /igbo-calendar/index.php should include this + render output.
 */
...
/* ---------------------------------------------------------
   Stable section wrappers (used by server render)
--------------------------------------------------------- */
if (!function_exists('igcal_month_open')) {
  function igcal_month_open(int $monthNum, string $monthLabel = ''): string {
    ...
    return '<section class="igcal-month mk-cal-month"'
      . igcal_attr('data-ig-month', (string)$monthNum)
      . igcal_attr('data-month', (string)$monthNum)
      . igcal_attr('data-month-index', (string)$monthNum)
      . igcal_attr('aria-label', $label)
      . '>';
  }
}
if (!function_exists('igcal_month_close')) {
  function igcal_month_close(): string { return '</section>'; }
}

/* ---------------------------------------------------------
   Public entry: render full year HTML (months)
--------------------------------------------------------- */
if (!function_exists('igbo_calendar_render_page')) {
  function igbo_calendar_render_page(int $gregYear, array $opts = []): string {
    ...
    } elseif (!empty($opts['preferred_new_year_marketday']) && is_string($opts['preferred_new_year_marketday'])) {
      $preferredStartDay = trim($opts['preferred_new_year_marketday']) ?: null;
    }
    ...
    $out  = '<div class="mk-cal" data-role="igbo-calendar-server-render">';
...
function igbo_get_today_iso(array $opts): string {
  ...
  if (!empty($opts['today_iso']) && is_string($opts['today_iso']) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $opts['today_iso'])) {
...
/* ---------------------------------------------------------
   New year start (approx): Feb 15..Feb 28 new moon window
--------------------------------------------------------- */
...
function igbo_build_months(DateTimeImmutable $yearStart, array $monthDays, array $monthNames, string $todayIso, array $anchor): array {
  ...
  for ($m = 1; $m <= 13; $m++) {
    ...
    $monthStart = $cursor;
    $monthName  = (string)($monthNames[$m] ?? ('Month ' . $m));

    for ($dayNo = 1; $dayNo <= $daysInMonth; $dayNo++) {
      $days[] = igbo_day_payload(
        $dayNo,
        $cursor,
        $todayIso,
        $anchor,
...
/* ---------------------------------------------------------
   Render month grid (4 columns, with offset blanks)
--------------------------------------------------------- */
function igbo_render_month_grid_with_offsets(array $m, array $anchor, string $todayIso): string {
  ...
  $html .= '<table class="mk-cal__table mk-cal-grid" role="table" aria-label="Igbo month grid">';
  $html .= '<thead><tr>';
  ...
  for ($r = 0; $r < $rows; $r++) {
    ...
    for ($c = 0; $c < 4; $c++) {
      ...
      if ($effective >= 0 && $effective < $daysInMonth) {
        ...
        $tdClass = 'mk-cal-cell';
        if ($isToday) $tdClass .= ' mk-cal-cell--today';

        $html .= '<td class="' . h($tdClass) . '"'
      . igcal_attr('data-iso', $iso)
      . igcal_attr('data-date', $iso)
      . igcal_attr('data-gregorian', $iso)
      . igcal_attr('data-date-label', (string)($d['date_label'] ?? ''))
      . igcal_attr('data-weekday', (string)($d['weekday'] ?? ''))
      . igcal_attr('data-market', (string)($d['market_day'] ?? ''))
      . igcal_attr('data-market-day', (string)($d['market_day'] ?? ''))
      . igcal_attr('data-igbo-day', (string)($d['day_no'] ?? ''))
      . igcal_attr('data-igbo-month', (string)($d['igbo_month_no'] ?? ''))
      . igcal_attr('data-igbo-month-name', (string)($d['igbo_month_name'] ?? ''))
      . igcal_attr('data-igbo-year', (string)($d['igbo_year'] ?? ''))
      . igcal_attr('data-greg-year', (string)($d['greg_year'] ?? ''))
      . igcal_attr('data-greg-month', (string)($d['greg_month'] ?? ''))
      . igcal_attr('data-greg-day', (string)($d['greg_day'] ?? ''))
      . igcal_attr('data-moon-pct', (string)($d['moon_pct'] ?? ''))
      . igcal_attr('data-moon-stage', (string)($d['moon_stage'] ?? ''))
      . igcal_attr('data-moon-icon', (string)($d['moon_icon'] ?? ''))
      . igcal_attr('data-moon-age', (string)($d['moon_age'] ?? ''))
      . igcal_attr('data-moon-frame', (string)($d['moon_frame'] ?? ''))
      . igcal_attr('data-moon-waxing', !empty($d['moon_waxing']) ? '1' : '0')
      . igcal_attr('data-is-today', $isToday ? '1' : '0')
      . '>'
...
/* ---------------------------------------------------------
   Day payload
--------------------------------------------------------- */
function igbo_day_payload(
  int $dayNo,
  DateTimeImmutable $date,
  string $todayIso,
  array $anchor,
  int $igboMonthNo = 0,
  string $igboMonthName = '',
  int $igboYear = 0
): array {
  ...
  $frame = igbo_moon_sprite_frame_from_phase((float)$phaseFrac);
  ...
  return [
    ...
    'greg_year'    => (int)$date->format('Y'),
    'greg_month'   => (int)$date->format('n'),
    'greg_day'     => (int)$date->format('j'),

    'igbo_year'       => $igboYear,
    'igbo_month_no'   => $igboMonthNo,
    'igbo_month_name' => $igboMonthName,
...
function igbo_render_cell(array $d, bool $isToday): string {
  ...
  $moonPct   = max(0, min(100, (int)($d['moon_pct'] ?? 0)));
  ...
  $html  = '<div class="mk-cal-cell__inner">';

  $html .= '<div class="mk-cal-cell__top">';
  $html .= '  <div class="mk-cal-day">Day ' . (int)$dayNo . '</div>';
  $html .= '  <div class="mk-cal-iso muted">' . h($dateIso) . '</div>';
  $html .= '</div>';
...
/* ---------------------------------------------------------
   Moon metrics (approx) + calibration knob + robust thresholds
--------------------------------------------------------- */
function igbo_moon_metrics(DateTimeImmutable $date): array {
  // returns: [ageDays, illumPct, stageLabel, icon, phaseFrac(0..1), waxing(bool)]
  $tz = new DateTimeZone('UTC');
  $dt = $date->setTimezone($tz)->setTime(12, 0, 0);

  $jd = igbo_julian_day($dt);

  // Calibration knob (days)
  $cal = 0.0;
...
function igbo_moon_stage_from_illum(int $pct, bool $waxing): string {
  // Robust thresholds (don’t depend on pct==50 exact)
  if ($pct <= 3)  return 'New Moon';
  if ($pct >= 97) return 'Full Moon';
